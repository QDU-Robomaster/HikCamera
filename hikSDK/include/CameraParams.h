...
#define MV_UNKNOW_DEVICE \
  0x00000000  ///< \~chinese 未知设备类型，保留意义       \~english Unknown Device Type,
              ///< Reserved
...
typedef struct _MV_GIGE_DEVICE_INFO_
{
  ...
  unsigned int nIpCfgCurrent;  ///< \~chinese       \~english IP
...
{
  unsigned char CrtlInEndPoint;  ///< \~chinese 控制输入端点         \~english Control
                                 ///< input endpoint
  unsigned char CrtlOutEndPoint;  ///< \~chinese 控制输出端点         \~english Control
                                  ///< output endpoint
  unsigned char
      StreamEndPoint;  ///< \~chinese 流端点               \~english Flow endpoint
  unsigned char
      EventEndPoint;        ///< \~chinese 事件端点             \~english Event endpoint
  unsigned short idVendor;  ///< \~chinese 供应商ID号           \~english Vendor ID Number
  unsigned short
      idProduct;  ///< \~chinese 产品ID号             \~english Device ID Number
  unsigned int
      nDeviceNumber;  ///< \~chinese 设备序列号           \~english Device Serial Number
  unsigned char chDeviceGUID[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 设备GUID号 \~english
                                                     ///< Device GUID Number
  unsigned char chVendorName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 供应商名字 \~english
                                                     ///< Vendor Name
  unsigned char chModelName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 型号名字 \~english
                                                    ///< Model Name
  unsigned char chFamilyName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 家族名字 \~english
                                                     ///< Family Name
  unsigned char chDeviceVersion[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 设备版本号
                                                        ///< \~english Device Version
  unsigned char
      chManufacturerName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 制造商名字 \~english
                                                 ///< Manufacturer Name
  unsigned char chSerialNumber[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 序列号 \~english
                                                       ///< Serial Number
  unsigned char chUserDefinedName[INFO_MAX_BUFFER_SIZE];  ///< \~chinese 用户自定义名字
                                                          ///< \~english User Defined Name
  unsigned int
      nbcdUSB;  ///< \~chinese 支持的USB协议        \~english Support USB Protocol
...
typedef struct _MV_CC_DEVICE_INFO_
{
  ...
  unsigned int
      nTLayerType;  ///< \~chinese 设备传输层协议类型，e.g. MV_GIGE_DEVICE\~english Device
                    ///< Transport Layer Protocol Type, e.g. MV_GIGE_DEVICE
...
{
  int64_t nReviceDataSize;  ///< \~chinese 已接收数据大小
                            ///< [统计StartGrabbing和StopGrabbing之间的数据量]\~english
                            ///< Received Data Size  [Calculate the Data Size between
                            ///< StartGrabbing and StopGrabbing]
...
#define MV_MAX_TLS_NUM \
  8  ///< \~chinese 最多支持的传输层实例个数\~english The maximum number of supported
...
typedef struct _MV_CC_DEVICE_INFO_LIST_
{
  ...
  MV_CC_DEVICE_INFO*
      pDeviceInfo[MV_MAX_DEVICE_NUM];  ///< \~chinese 支持最多256个设备\~english Support
                                       ///< up to 256 devices
...
typedef struct _MV_FRAME_OUT_INFO_
{
  ...
  unsigned int
      nDevTimeStampHigh;  ///< \~chinese 时间戳高32位\~english Timestamp high 32 bits
  unsigned int
      nDevTimeStampLow;  ///< \~chinese 时间戳低32位\~english Timestamp low 32 bits
  unsigned int
      nReserved0;  ///< \~chinese 保留，8字节对齐\~english Reserved, 8-byte aligned
  int64_t
      nHostTimeStamp;  ///< \~chinese 主机生成的时间戳\~english Host-generated timestamp

  unsigned int nFrameLen;

  unsigned int
      nLostPacket;  ///< \~chinese 本帧丢包数\~english Lost Pacekt Number In This Frame
...
/// \~chinese 图像结构体，输出图像指针地址及图像信息\~english Image Struct, output the
/// pointer of Image and the information of the specific image
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_
{
  ...
  enum MvGvspPixelType enPixelType;  ///< [IN]    \~chinese 输入数据的像素格式\~english
  ...
  unsigned int nBufferSize;  ///< [IN]    \~chinese 提供的输出缓冲区大小\~english Output
...
typedef struct _MV_SAVE_IMAGE_PARAM_T_EX_
{
  ...
  enum MV_SAVE_IAMGE_TYPE
      enImageType;  ///< [IN]     \~chinese 输出图片格式\~english Output Image Format
  unsigned int
      nJpgQuality;  ///< [IN]    \~chinese JPG编码质量(50-99]，其它格式无效\~english
                    ///< Encoding quality(50-99]，Other formats are invalid

  ///< [IN]    \~chinese Bayer格式转为RGB24的插值方法  0-最近邻 1-双线性 2-Hamilton
  ///< （如果传入其它值则默认为最近邻） [IN]    \~english Interpolation method of convert
  ///< Bayer to RGB24  0-nearest neighbour 1-bilinearity 2-Hamilton
...
typedef struct _MV_CC_ROTATE_IMAGE_PARAM_T_
{
  ...
  unsigned char*
      pDstBuf;  ///< [OUT] \~chinese 输出数据缓存           \~english Output data buffer
  unsigned int nDstBufLen;  ///< [OUT] \~chinese 输出数据长度           \~english Output
                            ///< data length
  unsigned int nDstBufSize;  ///< [IN]  \~chinese 提供的输出缓冲区大小   \~english
                             ///< Provided output buffer size

  MV_IMG_ROTATION_ANGLE enRotationAngle;  ///< [IN]  \~chinese 旋转角度 \~english Rotation
                                          ///< angle
...
typedef struct _MV_CC_FLIP_IMAGE_PARAM_T_
{
  ...
  MV_IMG_FLIP_TYPE
      enFlipType;  ///< [IN]  \~chinese 翻转类型               \~english Flip type
...
typedef struct _MV_PIXEL_CONVERT_PARAM_T_
{
  ...
  unsigned int nDstBufferSize;  ///< [IN]     \~chinese 提供的输出缓冲区大小\~english
...
typedef enum _MV_CC_GAMMA_TYPE_
{
  ...
  MV_CC_GAMMA_TYPE_SRGB2LRGB =
      4,  ///< \~chinese sRGB to linear RGB(仅色彩插值时支持，色彩校正时无效) \~english
          ///< sRGB to linear RGB
...
{
  MV_CC_GAMMA_TYPE
      enGammaType;    ///< [IN]  \~chinese Gamma类型              \~english Gamma type
...
typedef struct _MV_CC_FRAME_SPEC_INFO_
{
  /// \~chinese 设备水印时标      \~english Device frame-specific time scale
  unsigned int
      nSecondCount;  ///< [OUT] \~chinese 秒数                   \~english The Seconds
  unsigned int nCycleCount;  ///< [OUT] \~chinese 周期数                 \~english The
                             ///< Count of Cycle
  ...
  float
      fExposureTime;  ///< [OUT] \~chinese 曝光时间               \~english Exposure Time
  ...
  unsigned int
      nFrameCounter;  ///< [OUT] \~chinese 总帧数                 \~english Frame Counter
  unsigned int nTriggerIndex;  ///< [OUT] \~chinese 触发计数               \~english
                               ///< Trigger Counting
  ...
  unsigned short nFrameWidth;  ///< [OUT] \~chinese 水印宽                 \~english The
                               ///< Width of Chunk
  unsigned short nFrameHeight;  ///< [OUT] \~chinese 水印高                 \~english The
                                ///< Height of Chunk

  unsigned int
      nReserved[16];  ///<       \~chinese 预留                   \~english Reserved
...
{
  unsigned char*
      pSrcBuf;  ///< [IN]  \~chinese 输入数据缓存           \~english Input data buffer
  unsigned int
      nSrcLen;  ///< [IN]  \~chinese 输入数据大小           \~english Input data size
  ...
  unsigned int nDstBufSize;  ///< [IN]  \~chinese 提供的输出缓冲区大小   \~english
                             ///< Provided output buffer size
  unsigned int
      nDstBufLen;  ///< [OUT] \~chinese 输出数据大小           \~english Output data size
  enum MvGvspPixelType enDstPixelType;  ///< [OUT] \~chinese 输出的像素格式 \~english
                                        ///< Output pixel format

  MV_CC_FRAME_SPEC_INFO stFrameSpecInfo;  ///< [OUT] \~chinese 水印信息 \~english Frame
                                          ///< Spec Info
...
typedef enum _MV_GIGE_TRANSMISSION_TYPE_
{
  ...
  MV_GIGE_TRANSTYPE_LIMITEDBROADCAST =
      0x2,  ///< \~chinese 表示局域网内广播，暂不支持\~english Limited broadcast mode,not
            ///< support
  MV_GIGE_TRANSTYPE_SUBNETBROADCAST =
      0x3,  ///< \~chinese 表示子网内广播，暂不支持\~english Subnet broadcast mode,not
            ///< support
  MV_GIGE_TRANSTYPE_CAMERADEFINED = 0x4,  ///< \~chinese 表示从相机获取，暂不支持\~english
                                          ///< Transtype from camera,not support
  MV_GIGE_TRANSTYPE_UNICAST_DEFINED_PORT =
      0x5,  ///< \~chinese 表示用户自定义应用端接收图像数据Port号\~english User Defined
            ///< Receive Data Port
...
#define MV_MATCH_TYPE_NET_DETECT \
  0x00000001  ///< \~chinese 网络流量和丢包信息\~english Network traffic and packet loss
              ///< information
#define MV_MATCH_TYPE_USB_DETECT \
  0x00000002  ///< \~chinese host接收到来自U3V设备的字节总数\~english The total number of
              ///< bytes host received from U3V device
...
{
  unsigned int
      nType;  ///< \~chinese 需要输出的信息类型，e.g. MV_MATCH_TYPE_NET_DETECT\~english
              ///< Information type need to output ,e.g. MV_MATCH_TYPE_NET_DETECT
  void* pInfo;  ///< \~chinese 输出的信息缓存，由调用者分配\~english Output information
                ///< cache, which is allocated by the caller
...
/// \~en:Network traffic and packet loss feedback structure, the corresponding type is
...
/// \~chinese host收到从u3v设备端的总字节数，对应类型为 MV_MATCH_TYPE_USB_DETECT\~english
/// The total number of bytes host received from the u3v device side, the corresponding
...
typedef struct _MV_MATCH_INFO_USB_DETECT_
{
  int64_t nReviceDataSize;  ///< \~chinese 已接收数据大小
                            ...
                            ///< Received data size
  unsigned int
      nRevicedFrameCount;  ///< \~chinese 已收到的帧数\~english Number of frames received
...
typedef struct _MV_IMAGE_BASIC_INFO_
{
  ...
  unsigned int nSupportedPixelFmtNum;  ///< \~chinese 支持的像素格式种类\~english Support
                                       ///< pixel format
...
/// \~chinese 独占权限，其他APP只允许读CCP寄存器\~english Exclusive authority, other APP
/// is only allowed to read the CCP register
#define MV_ACCESS_Exclusive 1
/// \~chinese 可以从5模式下抢占权限，然后以独占权限打开\~english You can seize the
/// authority from the 5 mode, and then open with exclusive authority
#define MV_ACCESS_ExclusiveWithSwitch 2
/// \~chinese 控制权限，其他APP允许读所有寄存器\~english Control authority, allows other
/// APP reading all registers
#define MV_ACCESS_Control 3
/// \~chinese 可以从5的模式下抢占权限，然后以控制权限打开\~english You can seize the
/// authority from the 5 mode, and then open with control authority
...
/// \~chinese 可以从5的模式下抢占权限，然后以可被抢占的控制权限打开\~english You can seize
/// the authority from the 5 mode, and then open with seized control authority
#define MV_ACCESS_ControlSwitchEnableWithKey 6
/// \~chinese 读模式打开设备，适用于控制权限下\~english Open with read mode and is
/// available under control authority
...
typedef struct _MV_TRANSMISSION_TYPE_T
{
  ...
  unsigned short
      nDestPort;  ///< \~chinese 目标Port，组播模式下有意义\~english Destination port
...
typedef struct _MV_ACTION_CMD_INFO_T
{
  ...
  unsigned int
      bActionTimeEnable;  ///< \~chinese 只有设置成1时Action Time才有效，非1时无效
  ...
  unsigned int
      bSpecialNetEnable;  ///< [IN]  \~chinese 只有设置成1时指定的网卡IP才有效，非1时无效
...
typedef struct _MV_ACTION_CMD_RESULT_T
{
  ...
  int nStatus;  // 1.0x0000:success.
                ...
                // time reference. 4.0x8015:A device queue or packet data has overflowed.
...
typedef struct _MV_XML_FEATURE_Value_
{
  ...
  char strDescription[MV_MAX_XML_DISC_STRLEN_C];  ///< \~chinese节点描述
                                                  ///< 目前暂不支持\~english Node
...
typedef struct _MV_XML_FEATURE_Integer_
{
  ...
  enum MV_XML_AccessMode enAccessMode;  ///< \~chinese 访问模式\~english Access Mode
  int bIsLocked;  ///< \~chinese 是否锁定。0-否；1-是    目前暂不支持\~english Locked.
                  ///< 0-NO; 1-YES, NOT SUPPORT NOW
...
{
  char strDescription[MV_MAX_XML_DISC_STRLEN_C];  ///< \~chinese 节点描述
                                                  ///< 目前暂不支持\~english Node
                                                  ///< Description, NOT SUPPORT NOW
  char strDisplayName[MV_MAX_XML_NODE_STRLEN_C];  ///< \~chinese 显示名称\~english Display
                                                  ///< Name
...
typedef struct _MV_XML_FEATURE_Enumeration_
{
  ...
  char strCurrentSymbolic[MV_MAX_XML_SYMBOLIC_STRLEN_C];  ///< \~chinese
                                                          ///< 当前Symbolic索引\~english
...
typedef struct _MVCC_ENUMVALUE_T
{
  ...
  unsigned int
      nSupportedNum;  ///< \~chinese 数据的有效数据个数\~english Number of valid data
